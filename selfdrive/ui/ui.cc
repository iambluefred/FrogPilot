...
void update_model(UIState *s,
  ...
  bool isUnlimitedLength = isCustomRoadUI && params.getBool("UnlimitedLength");
  ...
  const float lane_line_width = std::stof(params.get("LaneLinesWidth")) / 12 * 0.1524;
  const float path_edge_width = std::stof(params.get("PathEdgeWidth"));
  const float path_width = std::stof(params.get("PathWidth")) / 10 * 0.1524;
  const float road_edge_width = std::stof(params.get("RoadEdgesWidth")) / 12 * 0.1524;
