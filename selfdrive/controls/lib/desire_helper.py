...
class DesireHelper:
  ...
  def update(self, carstate, lateral_active, lane_change_prob, md):
    ...
    if self.lane_detection and one_blinker:
      ...
      # Set the maximum lane threshold to 4.0 meters
      max_lane_threshold = 4.0
      ...
      if all([desired_edge.x, desired_edge.y, current_lane.x, current_lane.y]) and len(desired_edge.x) == len(current_lane.x):
        ...
        # Set lane_available to True if the lane width is between the thresholds
        self.lane_available = max_lane_threshold >= np.amax(lane_width) >= min_lane_threshold
      else:
