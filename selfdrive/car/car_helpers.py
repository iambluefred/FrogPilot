...
def get_frogpilot_params():
  params = Params()
  keys = [
    "FrogTheme", "FrogColors", "FrogIcons", "FrogSignals", "FrogSounds", "AdjustableFollowDistance", "BackButton", "Compass", "ConditionalExperimentalMode",
    "ConditionalExperimentalModeSpeed", "ConditionalExperimentalModeSpeedLead", "ConditionalExperimentalModeStopLights", "ConditionalExperimentalModeCurves", 
    "ConditionalExperimentalModeCurvesLead", "ConditionalExperimentalModeSignal", "CustomRoadUI", "LaneLinesWidth", "PathEdgeWidth", "PathWidth", "RoadEdgesWidth", 
    "UnlimitedLength", "DeviceShutdownTimer", "DisableAd", "DisableInternetCheck", "ExperimentalModeViaWheel", "FireTheBabysitter", "MuteDM", "MuteDoor", "MuteSeatbelt", 
    "MuteSystemOverheat", "NudgelessLaneChange", "LaneDetection", "OneLaneChange", "NumericalTemp", "PersonalTune", "ExperimentalPersonalTune", "RotatingWheel", "Sidebar", 
    "ScreenBrightness", "SteeringWheel", "WideCameraDisable"
  ]
...
def crash_log(candidate):
  ...
  frogpilot_values = []
  for k, v in frogpilot_params.items():
    if k == "PathWidth":
      value = float(v.decode('utf-8'))
      frogpilot_values.append("{:.0f}".format(value))
    else:
      frogpilot_values.append(v.decode('utf-8'))
  if is_connected_to_internet():
    sentry.capture_warning("Fingerprinted: %s\nLast updated: %s\nFrogPilot Params: {%s}" % (candidate, updated, ', '.join(frogpilot_values)))

def crash_log2(fingerprints, fw):
  if is_connected_to_internet():
    sentry.capture_warning("Car doesn't match any fingerprints.")
    sentry.capture_warning("Car doesn't match any fw.")
